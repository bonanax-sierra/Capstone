...
// Fetch barangays and associated data
$stmt = $pdo->query("
    SELECT barangay, COUNT(*) AS activity_count
    FROM activity_info
    GROUP BY barangay
");
...
echo "<script>const barangayData = " . json_encode($barangays) . ";</script>";
?>

<?php
$page_title = "Dashboard"; // Custom title for this page
...
<body>
  ...
  <div class="main-content">
    <div class="d-flex align-items-center justify-content-between mb-3">
      <div class="main-title d-flex align-items-center gap-2">
        <h1 class="m-0">Adolescent Risk Dashboard</h1>

        <!-- Notification Bell -->
        <div class="dropdown">
          <button class="btn btn-light position-relative" id="notifButton" data-bs-toggle="dropdown">
    ...
    <div class="alert alert-info mb-4">
      <h5><i class="bi bi-person-circle"></i> Welcome back, <?php echo htmlspecialchars($current_user['username']); ?>!
      </h5>
      <p class="mb-0">Here's your adolescent risk analytics overview.</p>
    </div>

    <!-- Modern Black & White Summary Cards -->
    <div class="row mb-4">

      <?php
      try {
        $total_assessments = $pdo->query("SELECT COUNT(*) FROM assessment")->fetchColumn();
        $recent_assessments = $pdo->query("SELECT COUNT(*) FROM assessment WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)")->fetchColumn();
        $total_activities = $pdo->query("SELECT COUNT(*) FROM activity_info")->fetchColumn();
        $risk_data = $pdo->query("SELECT risk_category, COUNT(*) as count FROM assessment GROUP BY risk_category")->fetchAll(PDO::FETCH_ASSOC);

        $high_risk = 0;
        foreach ($risk_data as $risk) {
          if ($risk['risk_category'] === 'High')
            $high_risk = $risk['count'];
        }
        ...
        <div class="col-md-3 mb-3">
          <div class="card h-100 bw-card">
            <div class="card-body d-flex justify-content-between align-items-center">
              <div>
                <h6 class="bw-title mb-1">Total Assessments</h6>
                <h3 class="bw-value"><?php echo number_format($total_assessments); ?></h3>
              </div>
        ...
        <!-- Recent (30 days) -->
        <div class="col-md-3 mb-3">
          <div class="card h-100 bw-card" style="background: linear-gradient(145deg, #f9f9f9, #e4e4e4);">
            <div class="card-body d-flex justify-content-between align-items-center">
              <div>
                <h6 class="bw-title mb-1">Recent (30 days)</h6>
                <h3 class="bw-value"><?php echo number_format($recent_assessments); ?></h3>
              </div>
        ...
        <!-- Active Activities -->
        <div class="col-md-3 mb-3">
          <div class="card h-100 bw-card" style="background: linear-gradient(145deg, #f4f4f4, #dedede);">
            <div class="card-body d-flex justify-content-between align-items-center">
              <div>
                <h6 class="bw-title mb-1">Active Activities</h6>
                <h3 class="bw-value"><?php echo number_format($total_activities); ?></h3>
              </div>
        ...
        <!-- High Risk Cases -->
        <div class="col-md-3 mb-3">
          <div class="card h-100 bw-card" style="background: linear-gradient(145deg, #f2f2f2, #e0e0e0);">
            <div class="card-body d-flex justify-content-between align-items-center">
              <div>
                <h6 class="bw-title mb-1">High Risk Cases</h6>
                <h3 class="bw-value"><?php echo number_format($high_risk); ?></h3>
              </div>
      ...
      <?php
        ...
        echo "<div class='col-12'><div class='alert alert-danger'>Error loading statistics: " . htmlspecialchars($e->getMessage()) . "</div></div>";
      }
      ?>
    </div>
    ...
    <div class="row mb-4">
      <div class="col-md-6">
        <div class="card">
          <div class="card-header">
            <h5><i class="bi bi-pie-chart"></i> Risk Level Distribution</h5>
          </div>
          <div class="card-body">
      ...
      <div class="col-md-6">
        <div class="card">
          <div class="card-header">
            <h5><i class="bi bi-bar-chart"></i> Top Problems</h5>
          </div>
          <div class="card-body">
            <?php
            try {
              // Fetch all problems as JSON
              $stmt = $pdo->prepare("SELECT current_problems FROM assessment WHERE JSON_LENGTH(current_problems) > 0");
              $stmt->execute();
              ...
              // Sort problems by count descending and take top 5
              arsort($problemCounts);
              $topProblems = array_slice($problemCounts, 0, 5, true);

              foreach ($topProblems as $problemName => $count) {
                $percentage = $total_assessments > 0 ? round(($count / $total_assessments) * 100, 1) : 0;
                ?>
                <div class="mb-3">
                  <div class="d-flex justify-content-between">
                    <span><?php echo htmlspecialchars($problemName); ?></span>
              ...
              echo "<div class='alert alert-warning'>Error loading problems data: " . htmlspecialchars($e->getMessage()) . "</div>";
            }
            ?>
          </div>
        </div>
    ...
    <div class="row">
      <div class="col-12">
        ...
        <div class="svg-map-container" id="riskMap">
          ...
          <svg version="1.1" id="svg1" width="480" height="491" viewBox="0 0 480 491" sodipodi:docname="sagay.svg"
            inkscape:version="1.4 (86a8ad7, 2024-10-11)" xmlns:inkscape="http://www.inkscape.org/namespaces/inkscape"
            xmlns:sodipodi="http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd" xmlns="http://www.w3.org/2000/svg"
            xmlns:svg="http://www.w3.org/2000/svg">
            <defs id="defs1">
              <inkscape:path-effect effect="bspline" id="path-effect32" is_visible="true" lpeversion="1.3"
                weight="33.333333" steps="2" helper_size="0" apply_no_weight="true" apply_with_weight="true"
                only_selected="false" uniform="false" />
              <inkscape:path-effect effect="bspline" id="path-effect30" is_visible="true" lpeversion="1.3"
                weight="33.333333" steps="2" helper_size="0" apply_no_weight="true" apply_with_weight="true"
                only_selected="false" uniform="false" />
              <inkscape:path-effect effect="spiro" id="path-effect28" is_visible="true" lpeversion="1" />
              <inkscape:path-effect effect="spiro" id="path-effect27" is_visible="true" lpeversion="1" />
              <inkscape:path-effect effect="bspline" id="path-effect26" is_visible="true" lpeversion="1.3"
                weight="33.333333" steps="2" helper_size="0" apply_no_weight="true" apply_with_weight="true"
                only_selected="false" uniform="false" />
              <inkscape:path-effect effect="bspline" id="path-effect22" is_visible="true" lpeversion="1.3"
                weight="33.333333" steps="2" helper_size="0" apply_no_weight="true" apply_with_weight="true"
                only_selected="false" uniform="false" />
            </defs>
            <sodipodi:namedview id="namedview1" pagecolor="#ffffff" bordercolor="#000000" borderopacity="0.25"
              inkscape:showpageshadow="2" inkscape:pageopacity="0.0" inkscape:pagecheckerboard="0"
              inkscape:deskcolor="#d1d1d1" showgrid="false" inkscape:zoom="3.1486762" inkscape:cx="297.58538"
              inkscape:cy="156.73254" inkscape:window-width="1245" inkscape:window-height="981" inkscape:window-x="665"
              inkscape:window-y="0" inkscape:window-maximized="0" inkscape:current-layer="g1" />
            <g inkscape:groupmode="layer" inkscape:label="Image" id="g1">
              <path
                ...
                id="path3" sodipodi:nodetypes="ccccccccccccccc" />
              <path
                ...
                id="path4" sodipodi:nodetypes="ccccccccccccccccccccccccccccccccccc" />
              <path
                ...
                id="path5" sodipodi:nodetypes="ccccccccccccccccccccc" />
              <path
                ...
                id="path6" sodipodi:nodetypes="cccccc" />
              <path
                ...
                id="path7" sodipodi:nodetypes="cccccccccccccccccccccccccccccccc" />
              <path
                ...
                id="path8" sodipodi:nodetypes="cccccccccccccccccccc" />
              <path
                ...
                id="path9" sodipodi:nodetypes="cccccccccccccc" />
              <path
                ...
                id="path10" sodipodi:nodetypes="ccccccccccccccccccccc" />
              <path
                ...
                id="path11" sodipodi:nodetypes="ccccccccccccccccccccccccccccc" />
              <path
                ...
                id="path12" sodipodi:nodetypes="ccccccccccccccccccccccccc" />
              <path
                ...
                id="path13" sodipodi:nodetypes="ccccccccccccccscccccccccccccccccccccccccccccc" />
              <path
                ...
                id="path14" sodipodi:nodetypes="ccccccccccccccccccccccccc" />
              <path
                ...
                id="path15" sodipodi:nodetypes="ccccccccccccccccccccccccccccccc" />
              <path
                ...
                id="path16" sodipodi:nodetypes="ccccccccccccccccccccccc" />
              <path
                ...
                id="path17" sodipodi:nodetypes="ccsccccccccccccccccccccccc" />
              <path
                ...
                id="path18" sodipodi:nodetypes="ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc"
                data-barangay="Old Sagay" />
              <path
                ...
                id="path19" sodipodi:nodetypes="cccscccccccccccccccccccccccccccccccc" data-barangay="Old Sagay" />
              <path
                ...
                id="path20" sodipodi:nodetypes="cccccccccccccccccccccccccccccc" />
              <g id="g35">
                <path
                  ...
                  id="path21" sodipodi:nodetypes="ccccccccccccccccccccccccccccccccccccccc" />
                <path
                  style="fill:#008fe5;fill-opacity:1;stroke:#ffffff;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                  ...
                  id="path22" inkscape:path-effect="#path-effect22"
              ...
              <path
                ...
                id="path23" sodipodi:nodetypes="ccccccccccccccccccccccccccccc" />
              <path
                ...
                id="path24" sodipodi:nodetypes="cccccccccccccccccccccccccc" />
              <g id="g34">
                <path
                  ...
                  id="path25" sodipodi:nodetypes="cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc" />
                <path
                  style="fill:#009273;fill-opacity:1;stroke:#ffffff;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                  ...
                  id="path26" inkscape:path-effect="#path-effect26"
                ...
                <path
                  ...
                  id="path27" inkscape:path-effect="#path-effect27"
                ...
                <path
                  ...
                  id="path28" inkscape:path-effect="#path-effect28"
              ...
              <g id="g33">
                <path
                  ...
                  id="path30" inkscape:path-effect="#path-effect30"
                ...
                <path
                  ...
                  id="path32" inkscape:path-effect="#path-effect32"
              ...
              <g inkscape:label="Clip" id="g36">
                <text xml:space="preserve"
                  ...
                  x="105.10003" y="373.689" id="text35" transform="translate(16.925457,-12.576073)">
                  <tspan x="105.10003" y="373.689" id="tspan1">Puey</tspan>
                </text>
              </g>
              <text xml:space="preserve"
                ...
                x="255.54239" y="-150.4698" id="text37" transform="rotate(90)">
                <tspan sodipodi:role="line" id="tspan37" x="255.54239" y="-150.4698">Campo Santiago</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="202.42477" y="289.33261" id="text38">
                <tspan sodipodi:role="line" id="tspan38" x="202.42477" y="289.33261">Makiling</tspan>
              </text>
              <g id="g4" style="overflow:hidden;fill:currentColor"
                ...
                <path
                  ...
                  fill="#9b7bff" id="path4-2" />
              </g>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="204.90167" y="102.19281" id="text39">
                <tspan sodipodi:role="line" id="tspan39" x="204.90167" y="102.19281">Himuga-an</tspan>
                <tspan sodipodi:role="line" x="204.90167" y="106.35947" id="tspan40">Baybay</tspan>
              </text>
              <text xml:space="preserve"
                ...
                x="250.5815" y="97.024902" id="text41">
                <tspan sodipodi:role="line" id="tspan41" x="250.5815" y="97.024902">Old</tspan>
                <tspan sodipodi:role="line" x="250.5815" y="100.35824" id="tspan42">Sagay</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="127.6727" y="320.29333" id="text43">
                <tspan sodipodi:role="line" id="tspan43" x="127.6727" y="320.29333">Colonia Divina</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="194.685" y="155.62096" id="text44">
                <tspan sodipodi:role="line" id="tspan44" x="194.685" y="155.62096">Paraiso</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="156.45044" y="-168.34624" id="text45" transform="rotate(90)">
                <tspan sodipodi:role="line" id="tspan45" x="156.45044" y="-168.34624">Fabrica</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="171.78166" y="262.61154" id="text46">
                <tspan sodipodi:role="line" id="tspan46" x="171.78166" y="262.61154">Baviera</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="161.02005" y="209.9295" id="text47">
                <tspan sodipodi:role="line" id="tspan47" x="161.02005" y="209.9295">Tadlong</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="191.82664" y="188.80951" id="text48">
                <tspan sodipodi:role="line" id="tspan48" x="191.82664" y="188.80951">Malubon</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="280.79141" y="108.09941" id="text49">
                <tspan sodipodi:role="line" id="tspan49" x="280.79141" y="108.09941">Taba-ao</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="340.61932" y="121.32083" id="text50">
                <tspan sodipodi:role="line" id="tspan50" x="340.61932" y="121.32083">Bulanon</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="371.42593" y="144.50517" id="text51">
                <tspan sodipodi:role="line" id="tspan51" x="371.42593" y="144.50517">Vito</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="326.32761" y="173.24741" id="text52">
                <tspan sodipodi:role="line" id="tspan52" x="326.32761" y="173.24741">Andres Bonifacio</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="299.33215" y="149.42789" id="text53">
                <tspan sodipodi:role="line" id="tspan53" x="299.33215" y="149.42789">Plaridel</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="268.20795" y="182.93402" id="text54">
                <tspan sodipodi:role="line" id="tspan54" x="268.20795" y="182.93402">General Luna</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="194.8438" y="225.49159" id="text55">
                <tspan sodipodi:role="line" id="tspan55" x="194.8438" y="225.49159">Campo </tspan>
                <tspan sodipodi:role="line" x="194.8438" y="229.65825" id="tspan56">Himuga-an</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="206.95282" y="253.59863" id="text57">
                <tspan sodipodi:role="line" id="tspan57" x="206.95282" y="253.59863">Bato</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="246.33913" y="235.82741" id="text58">
                <tspan sodipodi:role="line" id="tspan58" x="246.33913" y="235.82741">Lopez Jaena</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="237.08376" y="208.81792" id="text59">
                <tspan sodipodi:role="line" id="tspan59" x="237.08376" y="208.81792">Rizal</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="227.07956" y="150.85706" id="text60">
                <tspan sodipodi:role="line" id="tspan60" x="227.07956" y="150.85706">Poblacion II</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="173.22525" y="-250.66884" id="text61" transform="rotate(90)">
                <tspan sodipodi:role="line" id="tspan61" x="173.22525" y="-250.66884">Poblacion I</tspan>
              </text>
              <text xml:space="preserve"
                style="font-style:italic;font-variant:normal;font-weight:bold;font-stretch:normal;font-size:3.33333px;font-family:sans-serif;-inkscape-font-specification:'sans-serif Bold Italic';text-align:start;writing-mode:lr-tb;direction:ltr;text-anchor:start;fill:#000000;fill-opacity:1;stroke:none;stroke-width:1.12252;stroke-linecap:square;stroke-linejoin:round;paint-order:markers stroke fill"
                x="440.67166" y="60.952229" id="text62">
                <tspan sodipodi:role="line" id="tspan62" x="440.67166" y="60.952229">Molocaboc</tspan>
              </text>
              <g id="g4-7" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,248.79224,62.027167)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-1" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-1" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-5" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-2" />
              </g>
              <g id="g4-2" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,255.42743,83.219276)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-3" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-2" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-2" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-1" />
              </g>
              <g id="g4-6" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,298.71979,110.37451)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-18" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-9" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-27" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-9" />
              </g>
              <g id="g4-23" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,216.43196,130.69146)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-34" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-11" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-3" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-8" />
              </g>
              <g id="g4-79" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,195.13118,137.94437)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-31" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-98" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-6" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-5" />
              </g>
              <g id="g4-0" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,188.73726,245.33894)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-2" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-4" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-8" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-6" />
              </g>
              <g id="g4-06" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,213.47465,232.76843)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-13" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-8" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-9" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-3" />
              </g>
              <g id="g4-66" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,250.10595,154.63162)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-184" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-96" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-37" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-88" />
              </g>
              <g id="g4-5" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,244.4229,127.34347)" data-school-id="302767">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-9" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-84" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-0" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-7" />
              </g>
              <g id="g4-77" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,254.9381,145.68273)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-7" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-7" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-33" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-59" />
              </g>
              <g id="g4-26" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,376.36727,125.00517)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-6" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-0" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-38" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-0" />
              </g>
              <g id="g4-9" style="overflow:hidden;fill:currentColor"
                transform="matrix(0.01579114,0,0,0.01232821,327.46171,101.48124)">
                <path
                  d="m 537.42933,24.746667 c -9.38666,-0.512 -18.09066,-0.853333 -25.77066,-0.853334 -205.99467,0 -380.416,153.770667 -405.67467,357.546667 -12.458667,100.01067 12.62933,200.704 70.656,283.648 57.856,82.60267 143.53067,140.8 241.32267,163.84 L 428.032,831.31733 512,976.384 595.968,831.31733 606.03733,828.928 c 89.088,-20.992 169.472,-71.85067 226.47467,-143.53067 57.17333,-71.85066 88.74667,-161.792 88.74667,-253.44 0,-215.04 -168.61867,-393.898664 -383.82934,-407.210663 z"
                  fill="#070f1b" id="path1-4" />
                <path
                  d="M 538.79467,0.853333 C 529.92,0.341333 520.704,0 511.65867,0 293.71733,0 109.056,162.64533 82.261333,378.53867 c -27.306667,219.30666 116.565337,423.424 330.239997,473.6 L 475.136,960.34133 512,1024 548.864,960.34133 611.49867,852.13867 C 805.888,806.4 945.152,632.832 945.152,431.95733 945.152,204.288 766.80533,15.018667 538.79467,0.853333 Z M 606.03733,828.928 595.968,831.31733 512,976.384 428.032,831.31733 417.96267,828.928 C 320.17067,805.888 234.496,747.86133 176.64,665.088 118.61333,582.144 93.525333,481.45067 105.984,381.44 131.24267,177.664 305.664,23.893333 511.65867,23.893333 c 7.68,0 16.384,0.341333 25.77066,0.853334 215.21067,13.312 383.82934,192.341333 383.82934,407.210663 0,91.47734 -31.57334,181.58934 -88.74667,253.44 C 775.50933,756.90667 695.12533,807.936 606.03733,828.928 Z"
                  fill="#9b7bff" id="path2-78" />
                <path
                  d="m 767.488,313.17333 c -12.97067,-6.48533 -217.94133,-99.328 -217.94133,-99.328 0,0 -21.504,-10.92266 -32.42667,-10.92266 -10.92267,0 -32.42667,10.92266 -32.42667,10.92266 0,0 -205.14133,92.84267 -218.112,99.328 -12.97066,6.48534 0,12.97067 0,12.97067 0,0 174.93334,79.70133 207.18934,92.84267 25.94133,10.92266 32.42666,10.92266 43.17866,10.92266 10.92267,0 17.23734,0 43.17867,-10.92266 21.504,-10.92267 112.128,-51.712 166.22933,-75.60534 v 69.12 l -17.23733,17.23734 23.72267,23.72266 23.72266,-23.72266 -17.06666,-17.23734 V 339.11467 C 756.736,332.62933 767.488,326.144 767.488,326.144 c 0,0 12.8,-6.48533 0,-12.97067 z"
                  fill="#9b7bff" id="path3-8-35" />
                <path
                  d="m 517.12,474.624 c -10.92267,0 -17.23733,0 -43.17867,-10.92267 C 458.752,457.216 407.04,433.49333 359.59467,411.98933 v 157.52534 c 47.616,21.504 99.328,45.39733 114.34666,51.712 25.94134,10.92266 32.42667,10.92266 43.17867,10.92266 10.92267,0 17.23733,0 43.17867,-10.92266 15.18933,-6.48534 66.90133,-30.208 114.34666,-51.712 V 411.98933 c -47.616,21.504 -99.328,45.39734 -114.34666,51.712 C 534.35733,476.84267 527.872,474.624 517.12,474.624 Z"
                  fill="#9b7bff" id="path4-2-12" />
              </g>
            </g>
          </svg>
        </div>
  ...
  <div id="schoolTooltip"
    style="position:absolute; display:none; background:#333; color:#fff; padding:5px 10px; border-radius:5px; font-size:14px; pointer-events:none; z-index:1000;">
  </div>

  <div class="modal fade" id="barangayAssessmentsModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Barangay Assessments</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body" id="modalBarangayContent">
  ...
  <!-- Modal -->
  <div class="modal fade" id="topProblemsModal" tabindex="-1" aria-labelledby="topProblemsModalLabel"
    aria-hidden="true">
  ...
  <script>
    ...
    // ===== Build Modal Content for Schools =====
    function buildSchoolModalContent(data) {
      let content = `<strong>${data.school_name}</strong><ul>`;
      data.top_problems.forEach((item, i) => {
        content += `<li>${i + 1}. ${item.problem} (${item.count})</li>`;
      });
      content += `</ul><hr><p><strong>Suggested Actions:</strong><ul>`;
      data.top_problems.slice(0, 3).forEach(p => {
        const suggestion = suggestionsMap[p.problem] || "Guidance/Counseling";
        content += `<li>Offer <em>${suggestion}</em> for "${p.problem}"</li>`;
      });
      return content + `</ul></p>`;
    }

    // ===== Build Modal Content for Barangays =====
    function buildBarangayModalContent(data) {
      if (!data || data.length === 0) return "<p>No assessments found for this barangay.</p>";

      const barangayName = data[0]?.barangay || "Unknown";
      let content = `<h5>Barangay: <strong>${barangayName}</strong></h5>`;

      content += `<table class="table table-striped table-bordered mt-2">
    <thead>
      <tr>
        <th>#</th>
        <th>Student Name</th>
    ...
    <tbody>`;
      ...
      <td>${a.school_name || 'N/A'}</td>
      <td>${a.risk_category || 'Unknown'}</td>
      <td>${a.created_at}</td>
    </tr>`;
      });
    ...
    document.querySelectorAll("svg path[data-name]").forEach(path => {
      path.addEventListener("mousemove", e => showTooltip(e, path.dataset.name));
      path.addEventListener("mouseleave", hideTooltip);
    });

    // ===== School Groups with data-school-id =====
    const schoolMap = {};
    fetch("includes/get_all_schools.php")
      .then(res => res.json())
      .then(data => {
        data.forEach(school => schoolMap[school.school_id] = school.name);
        document.querySelectorAll("g[data-school-id]").forEach(g => {
          g.addEventListener("mousemove", e => {
            const name = schoolMap[g.dataset.schoolId];
            if (name) showTooltip(e, name);
          });
          g.addEventListener("mouseleave", hideTooltip);
          g.addEventListener("click", () => {
            const schoolId = g.dataset.schoolId;
            fetch(`includes/get_top_problems.php?school_id=${schoolId}`)
              .then(res => res.json())
              .then(data => {
                if (!data?.top_problems?.length) {
                  alert("No assessments for this school yet.");
                  return;
                }
                document.getElementById("modalProblemContent").innerHTML = buildSchoolModalContent(data);
                new bootstrap.Modal(document.getElementById("topProblemsModal")).show();
              })
              .catch(err => {
    ...
    // ===== Barangay SVG Paths =====
    document.querySelectorAll('#svg1 path[data-barangay]').forEach(path => {
      const barangayName = path.getAttribute('data-barangay');
      const data = barangayData.find(b => b.barangay === barangayName);

      if (data) {
        const maxCount = Math.max(...barangayData.map(b => b.activity_count));
        const intensity = data.activity_count / maxCount;
        path.style.fill = `rgba(220,53,69,${0.3 + 0.7 * intensity})`;

        path.addEventListener('mouseenter', e => showTooltip(e, `${barangayName}<br>Activities: ${data.activity_count}`));
        ...
        // Click → Fetch assessments
        path.addEventListener('click', () => {
          fetch(`includes/get_assessment.php?barangay=${encodeURIComponent(barangayName)}`)
            .then(res => res.json())
            .then(data => {
              document.getElementById("modalBarangayContent").innerHTML = buildBarangayModalContent(data);
              new bootstrap.Modal(document.getElementById("barangayAssessmentsModal")).show();
            })
            .catch(err => {
              console.error("Failed to fetch barangay assessments:", err);
              alert("Error fetching assessments for this barangay.");
            });
        });
      } else {
        path.style.fill = '#ccc';
    ...
    const riskData = <?php echo json_encode($risk_data); ?>; // High/Medium/Low counts
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: riskData.map(item => {
            if (item.risk_category === "High") return "#dc3545";
            if (item.risk_category === "Medium") return "#ffc107";
            return "#28a745";
          }),
          borderWidth: 1
        }]
